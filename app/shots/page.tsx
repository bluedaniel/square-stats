...
const COLUMNS: ColumnDef<Shot>[] = [
  ...
  },
  {
    accessorKey: "carry",
    header: "Carry (yd)",
    cell: ({ row }) => fmt(row.original.carry),
    meta: { statKey: "carry" },
  ...
  },
  {
    accessorKey: "offline",
    header: "Offline (yd)",
    cell: ({ row }) => fmtDir(row.original.offline),
    meta: { statKey: "offline" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "ballSpeed",
    header: "Ball Spd",
    cell: ({ row }) => fmt(row.original.ballSpeed),
    meta: { statKey: "ballSpeed" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "clubSpeed",
    header: "Club Spd",
    cell: ({ row }) => (row.original.clubSpeed ? fmt(row.original.clubSpeed) : "—"),
    meta: { statKey: "clubSpeed" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "smashFactor",
    header: "Smash",
    cell: ({ row }) => (row.original.smashFactor ? fmt(row.original.smashFactor, 2) : "—"),
    meta: { statKey: "smashFactor" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "launchAngle",
    header: "Launch°",
    cell: ({ row }) => fmt(row.original.launchAngle),
    meta: { statKey: "launchAngle" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "launchDirection",
    header: "Launch Dir",
    cell: ({ row }) => fmtDir(row.original.launchDirection),
    meta: { statKey: "launchDirection" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "spinRate",
    header: "Spin (rpm)",
    cell: ({ row }) => (row.original.spinRate ? fmt(row.original.spinRate, 0) : "—"),
    meta: { statKey: "spinRate" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "spinAxis",
    header: "Spin Axis",
    cell: ({ row }) => fmtDir(row.original.spinAxis),
    meta: { statKey: "spinAxis" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "backSpin",
    header: "Back Spin",
    cell: ({ row }) => fmt(row.original.backSpin, 0),
    meta: { statKey: "backSpin" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "sideSpin",
    header: "Side Spin",
    cell: ({ row }) => fmtDir(row.original.sideSpin),
    meta: { statKey: "sideSpin" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "apex",
    header: "Apex (yd)",
    cell: ({ row }) => fmt(row.original.apex),
    meta: { statKey: "apex" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "landingAngle",
    header: "Land°",
    cell: ({ row }) => fmt(row.original.landingAngle),
    meta: { statKey: "landingAngle" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "clubPath",
    header: "Club Path",
    cell: ({ row }) => fmtDir(row.original.clubPath),
    meta: { statKey: "clubPath" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "faceAngle",
    header: "Face Angle",
    cell: ({ row }) => fmtDir(row.original.faceAngle),
    meta: { statKey: "faceAngle" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "attackAngle",
    header: "Attack°",
    cell: ({ row }) => fmt(row.original.attackAngle),
    meta: { statKey: "attackAngle" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "dynamicLoft",
    header: "Dyn Loft",
    cell: ({ row }) => fmt(row.original.dynamicLoft),
    meta: { statKey: "dynamicLoft" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "impactHorizontal",
    header: "Impact H",
    cell: ({ row }) => fmtImpact(row.original.impactHorizontal),
    meta: { statKey: "impactHorizontal" },
    filterFn: numericFilter,
  },
  {
    accessorKey: "impactVertical",
    header: "Impact V",
    cell: ({ row }) => fmt(row.original.impactVertical),
    meta: { statKey: "impactVertical" },
    filterFn: numericFilter,
  },
];
...
export default function ShotsPage() {
  ...
  const [highlightMode, setHighlightMode] = useState<HighlightMode>(
    () =>
      (typeof window !== "undefined"
        ? (localStorage.getItem("highlightMode") as HighlightMode)
        : null) ?? "off"
  );
  ...
  const clubs = analysis ? ["All", ...new Set(analysis.shots.map((s) => s.club))] : ["All"];
  ...
  function colAvg(colId: string, skipZero = false): number | null {
    const vals = table
      .getRowModel()
      .rows.map((r) => r.getValue(colId) as number)
  ...
  function buildMarkdown(): string {
    ...
    const meta = [
      ...
      .filter(Boolean)
      .join(" · ");
    return [meta, "", toRow(headers), toRow(sep), ...dataRows.map(toRow)].join("\n");
  }
                ...
                return (
                  ...
                  >
                    {row.getVisibleCells().map((cell) => {
                      ...
                      const showGood =
                        status === "good" &&
                        (highlightMode === "positive" || highlightMode === "both");
                      const showBad =
                        status === "bad" &&
                        (highlightMode === "negative" || highlightMode === "both");
                      return (
                        <TableCell
                          key={cell.id}
                          className={[
      ...
      >
        <DialogContent className="sm:max-w-2xl max-h-[80vh] flex flex-col">
          ...
          <p className="text-xs text-muted-foreground">
            Copy this table and paste it into ChatGPT, Claude, or any AI assistant to analyse your
            shots.
          </p>
          <textarea
            readOnly
