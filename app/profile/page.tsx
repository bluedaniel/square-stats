...
function ProfilePageInner() {
  ...
  function handleIdealsClose(ideals: Partial<Record<ClubStatKey, IdealRange>> | null) {
    if (ideals && editIdealsClub) {
      setProfile((p) => {
        const next = {
          ...p,
          bag: p.bag.map((c) => (c.id === editIdealsClub.id ? { ...c, ideals } : c)),
        };
        saveProfile(next);
        return next;
      });
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
        ...
        <main className="flex-1 min-w-0 space-y-8">
          ...
          <section className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Your handicap sets the ideal ranges used to grade your shots — lower handicaps get
              tighter windows for carry, spin, and face-to-path.
            </p>
          </section>
            ...
            <div className="flex items-center justify-between">
              <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                ...
                {profile.bag.length > 0 && (
                  <span className="normal-case font-normal">({profile.bag.length})</span>
                )}
              </p>
        ...
        <aside className="hidden lg:flex flex-col gap-5 w-72 shrink-0 sticky top-6">
          <div className="rounded-xl border border-border p-5 space-y-3">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Square Stats is a free, offline desktop app for analysing Square launch monitor data.
              Drop in a CSV export and get instant shot analysis — no account, no cloud, no cost.
            </p>
            <p className="text-xs text-muted-foreground leading-relaxed">
          ...
          <div className="rounded-xl border border-border p-5 space-y-3">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Have an idea or found a bug? Open an issue on GitHub — all feedback is welcome, no
              matter how small.
            </p>
            <ExternalLink
          ...
          <div className="rounded-xl border border-border p-5 space-y-3">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              The app checks for updates automatically. You can also download the latest version
      ...
      <ClubIdealsModal
        key={editIdealsClub?.id ?? "none"}
        club={editIdealsClub}
        handicap={profile.handicap}
        onClose={handleIdealsClose}
      />
    </div>
  );
}
