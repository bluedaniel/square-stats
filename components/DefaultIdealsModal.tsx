...
import {
  CLUB_STAT_KEYS, CLUB_STAT_META, getDefaultIdeals, HANDICAP_OPTIONS,
  type Handicap,
} from "@/lib/profile";
      ...
      <DialogContent className="sm:max-w-5xl w-[90vw] max-h-[90vh] overflow-auto top-[5vh] translate-y-0">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr>
                <th className="text-left text-xs text-muted-foreground font-medium py-2 pr-4 whitespace-nowrap">Stat</th>
                {defaults.map(c => (
                  <th key={c.label} className="text-center text-xs text-muted-foreground font-medium py-2 px-3 whitespace-nowrap">
                    {c.label}
                  </th>
                ))}
              </tr>
                ...
                return (
                  <tr key={key} className="hover:bg-muted/30">
                    ...
                    {defaults.map(c => {
                      ...
                      return (
                        <td key={c.label} className="py-2 px-3 text-center tabular-nums whitespace-nowrap">
                          {r
