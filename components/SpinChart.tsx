    ...
    <Card>
      <CardHeader className="pb-2">
        ...
        <p className="text-xs text-muted-foreground">
          Colour by smash factor — <span style={{ color: "hsl(0 75% 50%)" }}>red</span> = poor · <span style={{ color: "hsl(60 75% 45%)" }}>amber</span> = ok · <span style={{ color: "hsl(120 75% 40%)" }}>green</span> = solid
        </p>
      </CardHeader>
      <CardContent>
        <div className="relative">
          <div className="absolute pointer-events-none border-l border-b border-border" style={{ left: 55, right: 16, top: 0, bottom: 38 }} />
                ...
                content={({ active, payload }) => {
                  ...
                  return (
                    <div className="bg-popover border border-border rounded-lg p-2.5 text-xs shadow-lg">
                      ...
                      <p>Spin <span className="tabular-nums font-medium">{s.spinRate.toFixed(0)} rpm</span></p>
                      <p>Carry <span className="tabular-nums font-medium">{s.carry.toFixed(1)} yd</span></p>
                      {s.smashFactor > 0 && (
                        <p style={{ color }}>Smash <span className="font-medium tabular-nums">{s.smashFactor.toFixed(2)}</span></p>
                      )}
                    </div>
                  );
                }}
