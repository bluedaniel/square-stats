...
export function LandingChart({ shots, currentShot }: Props) {
  ...
  const stdCarry  = n > 1 ? Math.sqrt(carries.reduce((a, b) => a + (b - meanCarry) ** 2, 0) / n) : 10;
  ...
  return (
    <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} className="w-full rounded">
      ...
      {([-2, -1, 1, 2] as const).map(n => n * displayMaxOffline / 2).map(off => (
        <line key={off}
          x1={toX(off)} y1={PAD_TOP} x2={toX(off)} y2={golferY}
          stroke="#3b6929" strokeWidth={0.5} />
      ))}
      <line x1={cx} y1={PAD_TOP} x2={cx} y2={golferY}
        stroke="#3b6929" strokeWidth={0.8} />
      ...
      {arcYards.map(d => {
        ...
        return (
          <g key={d}>
            ...
            <line x1={cx - 3} y1={labelY} x2={cx + 3} y2={labelY}
              stroke="#3b6929" strokeWidth={0.75} />
            <text x={PAD_SIDE - 3} y={labelY}
              textAnchor="end" fontSize={7} dominantBaseline="middle"
      ...
      {/* Dispersion ellipse */}
      <ellipse
        cx={toX(meanOff)} cy={toY(meanCarry)}
      ...
      {/* Other shots */}
      {shots.filter(s => s !== currentShot).map((s, i) => (
        <circle key={i}
          cx={toX(s.offline)} cy={toY(s.carry)}
          r={2.5} fill="white" opacity={0.8} />
      ))}

      {/* Current shot */}
      <circle cx={toX(currentShot.offline)} cy={toY(currentShot.carry)}
        r={4} fill="#facc15" />
      <text
        x={toX(currentShot.offline)} y={toY(currentShot.carry) - 7}
        textAnchor="middle" fontSize={7} fontWeight="bold" fill="#facc15"
      >
        #{currentShot.index}
      </text>
      ...
      {([-displayMaxOffline, -displayMaxOffline / 2, displayMaxOffline / 2, displayMaxOffline]).map(off => (
        <text key={off}
          x={toX(off)} y={SVG_H - 4}
          textAnchor="middle" fontSize={6}
          fill="rgba(255,255,255,0.45)">
          {Math.abs(off)}{off < 0 ? "L" : "R"}
        </text>
      ))}
    </svg>
  );
